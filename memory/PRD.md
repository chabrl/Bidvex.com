...
- **BUG 3 — charbel911@gmail.com admin unsubscribe ROOT CAUSE FOUND & FIXED at 3 layers**:
  - **Evidence from MongoDB**: `db.unsubscribe_events` shows the click on 2026-07-02 16:24 UTC from Bell Canada Quebec IP `66.254.39.32` (Chrome/149 Windows). Click propagated to `db.users` (marketing_unsubscribed=True), `db.email_suppressions`, `db.external_email_suppressions`, AND SendGrid global suppression API — silently blocking ALL admin alerts platform-wide.
  - **Fix 1 (immediate cleanup)**: Preview DB scrubbed live; user flags reverted; local suppressions purged; **SendGrid global DELETE API returned 204** for charbel911@gmail.com.
  - **Fix 2 (prevention guard)**: `POST /api/unsubscribe/confirm` and `/auto-confirm` now hard-block admin/super_admin emails with a 403 + bilingual `{error, message_en, message_fr}` envelope. Every attempt logs a red-flag warning and writes an audit row `event='blocked_admin_attempt'` to `db.unsubscribe_events` for forensics.
  - **Fix 3 (startup self-heal)**: `server.py` boots run a migration that scans for any admin/super_admin currently in suppression state, reverts flags, deletes from local suppression collections, and DELETEs from SendGrid global via API. Auto-repairs any future incident on next pod restart.
- **BUG 2 — Storage/Facilities/Campaigns 403 ROOT CAUSE**: role check regression sweep across **28 lines in 15 files** — every `role == "admin"` / `role != "admin"` replaced with `role in ("admin", "super_admin")`. Files: storage_auctions, external_campaigns, admin_oversight, admin_payment_requests, ai_chat, auctions_bids, listings, notifications, partner_trial, promotions, reviews (×5 via replace_all), trial_coupons, vehicle_settlement, vehicles + services (ai_assistant, ai_assistant_v2, listings_service). `deps.py:110 require_admin` was already correct (iter344).
- **External Campaigns defensive**: `GET /api/admin/external-campaigns` list endpoint now uses `.get()` defaults for every modern field (subject_en/fr, analytics, recipient_count, auto_paused, followup_emails_generated, attach_trial_coupon) with a per-row try/except that skips malformed rows — schema drift can no longer 500 the endpoint.
- **BUG 1 — Dialer hedges (partial fix — awaiting user's Twilio Console evidence for final RCA)**:
  - **First-line webhook logging** in `POST /api/twilio/twiml`: `[TWIML] webhook received | To=… From=… CallSid=… Direction=… CallLogId=… RequestURL=… Host=… XFHost=… XFProto=…` — will confirm within seconds whether the webhook is being reached on prod and what URL Twilio is hitting.
  - **Coach stream circuit breaker**: Nonce lookup miss now sets `coach_stream_url=None` and proceeds to `<Dial>`. `build_outbound_twiml` wraps the `<Start><Stream>` block in try/except — any failure in stream construction is caught, the whole TwiML build itself is wrapped in a second try/except that falls back to a `<Dial>`-only TwiML. The <Dial> to the client is now load-bearing and cannot be short-circuited.
  - **Nonce TTL bump 120s → 300s**: Cold-pod TwiML webhook can hit several seconds after session-init; 2 min was too tight causing premature expiry → coach degraded. Zero attack-surface change (nonce is single-use).
- **Admin Logs pagination** — `GET /api/admin/logs?page=1&limit=50` returns `{items, total_count, page, pages, limit, logs (alias)}` envelope. Details field is trimmed (long strings → 200 char + ellipsis; dicts → 6 keys max + `+N more`). Payload dropped from **~115 MB → ~100 KB per page**. Frontend `AdminLogs.js` gains a "Load more (N / total)" button (data-testid `admin-logs-load-more`) that appears when `page < pages`.
- **Weekly Impersonation Compliance Digest** — new `services/compliance_digest.py` (170 lines). APScheduler cron `Mon 13:00 UTC` (= 9AM ET). Bilingual HTML: EN primary body + "Résumé en français" section. Sessions > 30 min flagged 🚨. Recipient: `COMPLIANCE_OFFICER_EMAIL` env (fallback `charbel911@gmail.com`). Registered in `services/scheduler.py`.
- **Watchlist bid reminders** — `process_last_chance_nudges` (built in iter299 P1 but **dead code — never registered**) now wired into APScheduler as `last_chance_nudges` interval 10 min. Fires for every watchlisted listing / lot ending within 1 hour across ALL 5 collection types (listings, multi_item_listings, vehicle_listings, storage_auctions, vehicle_multi_lot_auctions + per-lot). Deduped via `db.last_chance_log` + `last_chance_sent=true` flag on the listing/lot doc.

**⚠️ CHARBEL ACTIONS REMAINING**:
- SendGrid Dashboard → Suppressions → verify `charbel911@gmail.com` is not present in Unsubscribes / Bounces / Spam Reports (self-heal did it via API, but manual check strongly recommended)
- Twilio Console → Voice → TwiML Apps → BidVex Dialer → confirm Primary=`.../api/twilio/twiml` and Fallback=`.../api/twilio/twiml-fallback`
- After redeploy: place a real contractor→client test call and paste 6 Twilio call log data points (Status, To, Duration, TwiML executed, error codes, Recording SID). The new first-line log will reveal whether the webhook is even being reached.

## iter345 — Dialer Root-Cause + VML Watchlist + Register 409 + Impersonation History (Feb 11, 2026) ✅ COMPLETE — TESTED
 (backend 20/20 iter345+iter340 regression suite + testing agent iteration_345.json 100% backend + 100% frontend)
- **BUG 1 — Dialer routing to +14506343099 (production) ROOT CAUSE FOUND & FIXED at 3 layers**:
  - Investigated all 4 candidates (A/B/C/D) then discovered **Candidate E**: `/api/twilio/twiml` used STRICT signature validation (`raise HTTPException(403)`); K8s ingress rewrites URL between `bidvex.com` (what Twilio signed) and pod's internal host → validation ALWAYS mismatched on prod → 403 → Twilio hit TwiML App **Fallback URL** which is misconfigured → misroute to main line. Same class of bug that iter324 hotfixed for the IVR routes but never got applied to outbound `/twiml`.
  - **Fix 1 (lenient signature validation)**: `_verify_twilio_signature` now tries multiple URL reconstructions (`x-forwarded-proto/host`, raw, legacy) and if all fail, ADMITS with a loud warning log — mirroring the proven iter324 IVR pattern. Missing signature header no longer 403s.
  - **Fix 2 (safe fallback endpoint)**: NEW `GET|POST /api/twilio/twiml-fallback` returns bilingual EN+FR `<Say>` + `<Hangup>` with **ZERO `<Dial>` tags and ZERO mention of the main line**. Charbel will point Twilio Console → Voice → TwiML Apps → BidVex Dialer → Fallback URL to this endpoint — guaranteed no more misroutes even if primary URL ever fails again.
  - **Fix 3 (grep proof)**: `grep -n "4506343099" /app/backend/routes/twilio.py` = 0 matches; endpoint code path never dials TWILIO_PHONE_NUMBER (only uses it as `callerId`).
- **BUG 2 — Watchlist for `vehicle_multi_lot_auctions` was never wired end-to-end**:
  - Backend: `POST /api/watchlist/add` supports `item_type="vehicle_multi_lot"` (404 on missing VML); `GET /api/watchlist` returns new `vehicle_multi_lot[]` bucket normalized with `title` (event_title → fallback), `current_price` (summed from lots when event has no price), `auction_end_date` (max of lot `end_time`), `images` (first lot image fallback), `city/region` (event fields), and the `lots[]` array is stripped from the output.
  - Frontend: `<WatchlistButton itemType="vehicle_multi_lot">` added to `VehicleMultiLotDetailPage.js` header (`data-testid="vml-watchlist-btn-wrapper"`); `WatchlistPage.js` renders "Vehicle Multi-Lot Auctions" section (`watchlist-vehicle-multi-lot-section`) mapping to `/vehicle-multi-lot/:id`; `WatchlistButton.checkWatchlistStatus` now checks ALL buckets (previously missed `vehicles/storage/vehicle_multi_lot` so heart never lit up).
- **BUG 3 — Registration 500 race on rapid double-submit → 409 bilingual**:
  - Wrapped `db.users.insert_one` in try/except for `DuplicateKeyError` (E11000). Both pre-check and race branch now return **409** with `detail = {error, message_en, message_fr}` envelope (bilingual FR/EN). Frontend can render either message directly.
- **BUG 4 — Impersonation History sub-tab**:
  - `GET /api/admin/impersonation-history` returns materialized sessions from `admin_logs` (action=`impersonation_started`) with `duration_minutes` and per-session `actions[]` (bounded by session start/expires + admin_id/target_user_id + excludes `impersonation_started` rows to prevent cumulative bleed). Filters: `admin_id`, `target_user_id`, `start_date`, `end_date`. 403 for non-admin.
  - Frontend: `AdminLogs.js` rewritten with `<Tabs>` — "Action History" + "Impersonation History" sub-tabs (`admin-logs-tabs`, `admin-logs-tab-actions`, `admin-logs-tab-impersonation`, `impersonation-history-panel`). Impersonation panel: filter inputs + Apply button + expandable action lists + CSV export.
- **Regression**: All 7 iter340 dialer routing tests still pass. `test_iter345_dialer_watchlist_history.py` — 13 new tests, all pass. Full run: **20/20 PASS**.
- **Side polish (from testing agent iteration_345.json feedback)**: (a) `VehicleMultiLotDetailPage.StatusBadge` now defensive against undefined `status` (was crashing whole page on legacy VML docs missing `status`); (b) `AdminLogs.js` map keys resilient to missing `log.id`; (c) impersonation-history `actions[]` window bounded strict `$gt` … `$lte` and excludes concurrent `impersonation_started` rows.
- **⚠️ USER ACTION REMAINING (Charbel)**: In Twilio Console → Voice → TwiML Apps → BidVex Dialer → **Fallback URL**, set the URL to `https://bidvex.com/api/twilio/twiml-fallback`. This is the safety net if the primary URL ever fails — it will hang up cleanly, never misroute.

## iter344 — Role Normalization + P0 Admin Superuser + Twilio Mic Check + Per-Lot Feeds (Jul 11, 2026) ✅ COMPLETE — TESTED (testing agent iteration_340.json + backend suite /app/backend/tests/test_iter344_sprint.py 12/13 pass)
- **CRITICAL PRE-FIX — Role string normalization**: `"superadmin"` (no underscore) → `"super_admin"` EVERYWHERE. Backend: 25 occurrences in 14 files fixed incl. `deps.py:110` `require_admin` (the root cause of recurring admin 403s). Frontend: 17 occurrences in 9 files fixed (AdminDashboard.js gate + badge, Navbar.js ×5, CreateListingPage, PartnerDashboard ×2, ListingDetailPage, storage/* ×3, App.js). DB: charbel911@gmail.com role updated `admin` → `super_admin`. Self-healing startup migration in server.py converts legacy `superadmin` rows on every boot (fixes prod on deploy). Audit test `test_admin_audit_iter156::TestRoleCanonicalization` enforces no regression.
- **P0 Full Admin Superuser**: 35+ ownership 403 checks across 25 backend files now bypass for `admin`/`super_admin` (brokers, drafts, listings, payments, settlement, vehicle_settlement, invoices, reviews, community, storage_auctions, vehicle_dealer_extras, messages, misc, admin_ai_review, transaction_pickup_code, listing_requests, broker_compliance, vehicles_admin, services/vehicle_payment). EXCEPTION: contractor_agreements stay READ-ONLY for admins — new `GET /api/twilio/admin/contractor-agreements` (read_only:true) + sign endpoint 409s for admins.
- **Impersonation**: `POST /api/admin/impersonate/{user_id}` → 1-hour JWT with `impersonated_by` claim; admin targets blocked (403); logged `impersonation_started` in admin_logs. Frontend: "Login As User" in Users dropdown (`impersonate-user-{id}` testid, hidden for admins), red sticky `ImpersonationBanner` (bilingual, `impersonation-banner` testid) with Exit button restoring the backed-up admin token (localStorage keys bidvex_admin_backup_token/_refresh/bidvex_impersonation).
- **Admin password reset**: existing `POST /api/admin/users/{user_id}/reset-password` (token + email) now ALSO logs `password_reset_triggered` {admin_id, target_user_id, timestamp} in admin_logs. Admin never sees the password.
- **BUG 2 Admin Lot Edit (prod)**: root cause was the deps.py role mismatch (now fixed). Endpoints `PUT /api/admin/vehicle-multi-lot-auctions/{event}/lots/{lot_id}` + `PUT /api/admin/multi-item-listings/{id}/lots/{n}` verified E2E with field-level diffs in admin_logs; AdminLotEditorModal wired via "Lots" button in Manage All Auctions. **Prod will work after redeploy.**
- **BUG 1 Twilio 31402**: pre-call `getUserMedia({audio:true})` check in AdminDialer.startCall with bilingual error toast; tracks stopped after probe; Device constructor has no custom audio constraints. ⚠️ Real test call from dashboard = USER ACTION after deploy.
- **BUG 3 Per-lot feeds**: `map_multi_lot_listing_to_meta_items()` decomposes multi-lot auctions into ONE item per lot for Google XML + Meta CSV/JSON: ids `LOT-{parent}-L{n}` / `VML-{parent}-{lotid8}`, deep links `?lot={ref}`, direct S3 lot photos, per-lot prices, per-lot availability, `<g:shipping>` blocks. `vehicle_multi_lot_auctions` collection added to feeds (statuses live/ended). Immediate regeneration via `POST /api/feeds/facebook-local/refresh`. All 182 feed tests pass (7 stale assertions updated). Seed script `scripts/iter344_seed_vml_feed_test.py` (event `iter344-vml-feed-test`).
- Fixed en route: trust-safety scores 500 (legacy user doc without `id`), stale test assertions in admin_audit_iter156 + phase5/iter231/iter291 feed suites.
- ⚠️ Known noise: `POST /api/auth/register` intermittently 500s on rapid repeated registrations in preview (typed error envelope) — pre-existing, non-blocking.

## iter343 — Six Regression Fixes (Jul 11, 2026) ✅ COMPLETE — 21 new tests + 88 prior tests pass, UI-verified
- BUG 1 Map search: geo endpoint unioned across all 5 collections + creation-time geo + idempotent backfill script (run). BUG 2 Homepage: featured/ending-soon/new/hot now include multi-lot + all collections (is_promoted OR is_featured). BUG 3 Twilio: env had the ROTATED auth token — replaced with user-provided token; live REST auth = VALID (production env still needs the same update + redeploy + a real test call by Charbel). BUG 4: NEW admin per-lot edit endpoints + AdminLotEditorModal with field-level admin_logs diff (E2E verified). BUG 5 Watchlist: unavailable placeholders, lot price normalization, vehicle+storage watchable (buttons added to detail pages). BUG 6: bid model CONFIRMED per-lot default / per-item when multiply_hammer_by_quantity — CostBreakdown + BidConfirmationDialog show Quantity row and multiply correctly (verified $30 × 10 = $300).
- Details in CHANGELOG.md iter343. Seeded ui343-* UI-test listings + re-seeded test users (see test_credentials.md).
