...
def test_proof2_cash_qc_800_with_deposit():
    ...
    # iter443: Buyer's BidVex charge is 5% BP + Stripe recovery + tax = 47.67
    assert p["buyer_invoice"]["total"] == 47.67
    # Deposit ($100) fully covers → remaining = 0.
    assert p["buyer_invoice"]["remaining_after_deposit"] == 0.00
    # iter443: fee_payer is BUYER, not facility.
    assert p["buyer_invoice"]["fee_payer"] == "buyer"
    # iter443: facility is NEVER invoiced — all zeros on facility_invoice.
    assert p["facility_invoice"]["bidvex_platform_fee"] == 0.00
    assert p["facility_invoice"]["stripe_recovery"] == 0.00
    assert p["facility_invoice"]["tax"] == 0.00
    assert p["facility_invoice"]["facility_owes_bidvex"] == 0.00
    assert p["facility_invoice"]["facility_net"] == 800.00


# ─────────────────────────────────────────────────────────────
# PROOF 3 — E-Transfer, $1,500 hammer, ON, no deposit (iter443)
# ─────────────────────────────────────────────────────────────
def test_proof3_etransfer_on_1500_no_deposit():
    ...
    # iter443: Buyer's BidVex charge is 5% BP + Stripe recovery + tax = 87.55
    assert p["buyer_invoice"]["total"] == 87.55
    # iter443: fee_payer is BUYER.
    assert p["buyer_invoice"]["fee_payer"] == "buyer"
    # iter443: facility is NEVER invoiced.
    assert p["facility_invoice"]["bidvex_platform_fee"] == 0.00
    assert p["facility_invoice"]["stripe_recovery"] == 0.00
    assert p["facility_invoice"]["tax"] == 0.00
    assert p["facility_invoice"]["facility_owes_bidvex"] == 0.00
    assert p["facility_invoice"]["facility_net"] == 1500.00
