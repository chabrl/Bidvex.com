...
async def active_banners(
    ...
    ticket 209107 — anonymous callers (no auth token) are now allowed
    through; they only see untargeted ("all") active banners since
    tier/province/custom/new_users eligibility requires a hydrated user.
    """
    db = get_db()
    now_iso = datetime.now(timezone.utc).isoformat()
    ...
    # Hydrate user once for eligibility checks (skip for anonymous callers).
    user = None
    if current_user:
    ...
    for promo in candidates:
        ...
        if user:
            ...
            target = (promo.get("target") or (promo.get("target_config") or {}).get("target") or "all").lower()
            if target != "all":
                continue
        banners.append({
            "id": promo["id"],
