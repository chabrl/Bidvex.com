"""
iter310 — Admin user management routes (split out of admin_user_actions.py)
============================================================================

Handles core user/business account CRUD workflows under the
Admin → Users tab. Tier/billing/subscription endpoints live in
`admin_user_billing.py`; the two are bundled back together by the
`admin_user_actions.py` shim so existing imports in `server.py` keep
working.

Routes (all mounted at `/api/admin/users/{user_id}/...`):
  • POST  /send-notification                     — bilingual notify (email + in-app)
  • POST  /request-documents                     — request docs + audit + email
  • GET   /document-requests                     — list a user's document requests
  • PATCH /profile                               — edit basic profile fields
  • POST  /reset-password                        — issue a password-reset email
  • POST  /convert-to-demo                       — flip the demo-account flag
  • GET   /email-journey                         — read the 6-email onboarding journey
  • POST  /email-journey/trigger/{email_number}  — manual journey email re-send
  • POST  /email-journey/cancel                  — cancel remaining journey emails
  • POST  /email-journey/reset                   — restart journey from email 1
  • POST  /bidding-suspension                    — set/lift bidding suspension
"""
...
async def admin_send_notification(
    ...
    if payload.send_via in {"in_app", "both"}:
        ...
        await db.notifications.insert_one({
            ...
            "data": {"attached_transaction_id": payload.attached_transaction_id},
    ...
    return {"success": True, "email_sent": sent_email, "in_app_id": in_app_id}
...
async def admin_list_user_document_requests(
    ...
    db = get_db()
    rows = await db.user_document_requests.find(
...
# ─── Edit Profile / Reset Password / Convert to Demo ──────────────────
