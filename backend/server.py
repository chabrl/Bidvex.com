from fastapi import FastAPI, APIRouter, HTTPException, Depends, Request, Header, status, WebSocket, WebSocketDisconnect, Query
...
async def add_payment_method(data: PaymentMethodCreate, current_user: User = Depends(get_current_user)):
    try:
        ...
        intent = stripe.PaymentIntent.create(
            ...
            confirm=True,
            return_url='https://visual-lab-7.preview.emergentagent.com'
        )
...
async def get_affiliate_stats(current_user: User = Depends(get_current_user)):
    ...
    return {
        ...
        "referral_link": f"https://visual-lab-7.preview.emergentagent.com/auth?ref={current_user.affiliate_code}",
