...
# Message model used by WebSocket handlers — canonical definition in models/message_models.py
...
# ========== PAYMENTS/CHECKOUT (Moved to routes/payments.py) ==========
# ========== STRIPE WEBHOOKS (Moved to routes/webhooks.py) ==========
# ========== DASHBOARD (Moved to routes/dashboard.py) ==========
...
# ========== PAYMENT METHODS (Moved to routes/payments.py) ==========
...
# ========== LOCATION SEARCH (Moved to routes/marketplace.py) ==========
...
# ========== MESSAGING CORE — Moved to routes/messages.py ==========



# ========== TAX PROFILE (Moved to routes/profiles.py) ==========
...
# ========== MESSAGING — Moved to routes/messages.py ==========




# ========== UPLOAD DOCUMENT (Moved to routes/profiles.py) ==========
...
# Conversation online-status — Moved to routes/messages.py
...
# MESSAGING OVERSIGHT — Moved to routes/messages.py
...
# ========== WATCHLIST ENDPOINTS (Moved to routes/watchlist.py) ==========
...
# ========== CAROUSEL & STATS (Moved to routes/carousel.py) ==========
...
# ========== DATA MANAGEMENT / GDPR (Moved to routes/profiles.py) ==========
...
# ========== USER STATS (Moved to routes/profiles.py) ==========

# ========== SITE CONFIGURATION & HERO BANNERS (Moved to routes/site_config.py) ==========
...
try:
    ...
    set_dashboard_auth(get_current_user)
    set_profiles_auth(get_current_user)
    ...
    # Inject WebSocket manager and SMS service into bids router
    set_ws_manager(manager if 'manager' in dir() else None)
    try:
        from services.sms_notifications import get_sms_notification_service
        set_sms_service_getter(get_sms_notification_service)
    except ImportError:
        logger.warning("SMS notification service not available for bids router")
    ...
    logger.info("Bids router loaded")
    
    api_router.include_router(listings_router)
    logger.info("Listings router loaded")
    ...
    # Include marketplace router for browsing/search/filter
    api_router.include_router(marketplace_router)
    logger.info("Marketplace router loaded")
    
    # Note: users, marketing routers contain endpoints
    ...
    # Include admin router (partner management, email settings now live here)
    ...
    # Import and include modular route modules
    try:
        from routes.ai_chat import ai_chat_router, set_ai_chat_db
        set_ai_chat_db(db)
        api_router.include_router(ai_chat_router)
        logger.info("✅ AI Chat router loaded")
    except Exception as e:
        logger.warning(f"⚠️ Could not load AI chat router: {e}")

    try:
        from deps import set_db as set_deps_db
        set_deps_db(db)
        from routes.fees import fees_router
        api_router.include_router(fees_router)
        logger.info("✅ Fees router loaded")
    except Exception as e:
        logger.warning(f"⚠️ Could not load fees router: {e}")
    ...
    try:
        ...
        logger.warning(f"⚠️ Could not load notifications router: {e}")
    ...
    try:
        from routes.messages import messages_router, set_messages_db, set_message_managers
        set_messages_db(db)
        set_message_managers(message_manager, manager)
        api_router.include_router(messages_router)
        logger.info("Messages router loaded")
    except Exception as e:
        logger.warning(f"Could not load messages router: {e}")

    try:
        from routes.tax_reports import tax_router as tax_reports_router, set_tax_db
        set_tax_db(db)
        api_router.include_router(tax_reports_router)
    ...
    try:
        from routes.carousel import carousel_router, set_carousel_db
        set_carousel_db(db)
        api_router.include_router(carousel_router)
        logger.info("Carousel & Stats router loaded")
    except Exception as e:
        logger.warning(f"Could not load carousel router: {e}")

    try:
        from routes.site_config import site_config_router, set_site_config_db
        set_site_config_db(db)
        api_router.include_router(site_config_router)
        logger.info("Site Config router loaded")
    except Exception as e:
        logger.warning(f"Could not load site config router: {e}")
...
# ========== PARTNER ADMIN (Moved to routes/admin.py) ==========
# verify, reject, toggle, premium-rate, email settings, partner applications
...
# ========== MARKETPLACE (Moved to routes/marketplace.py) ==========
# marketplace/items, filter-counts, location search, promoted-listings, track-click
...
# ========== TAX INFO + STRIPE CONNECT (Moved to routes/profiles.py) ==========
...
# ========== PROMOTED LISTINGS (Moved to routes/marketplace.py) ==========
...
# ========== NOTIFICATION ENDPOINTS (Moved to routes/notifications.py) ==========
...
# ========== AI CHAT ENDPOINTS (Moved to routes/ai_chat.py) ==========

# ==================== SITE CONTENT MANAGEMENT (CMS) ====================
# Dynamic footer links and legal pages management

class LegalPageContent(BaseModel):
    title: str
    content: str
    link_type: Optional[str] = "page"  # page, mailto, chatbot
    link_value: Optional[str] = None

class LegalPagesConfig(BaseModel):
    how_it_works: Dict[str, LegalPageContent]
    privacy_policy: Dict[str, LegalPageContent]
    terms_of_service: Dict[str, LegalPageContent]
    support: Dict[str, LegalPageContent]

@api_router.get("/site-config/legal-pages")
...
# ========== FEE ENDPOINTS (Moved to routes/fees.py) ==========
