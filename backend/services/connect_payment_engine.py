...
def calculate_connect_checkout(
    ...
    # ── iter482 P5 canonical payer-bears-fee gate (L-1 CLEARED) ──
    # Compute the buyer-facing Stripe processing RECOVERY (gross-up) via
    # the single canonical engine.  This is now the ONLY source of truth
    # for the buyer's payment_processing amount on this path.
    from services.payment_cost_engine import estimate as _pp_estimate, PayerRole as _PP_ROLE
    # Base amount for the recovery calc = buyer's total BEFORE Stripe
    # recovery is added (hammer + BP + taxes).  Strip legacy
    # ``bi.stripe_recovery`` (from PricingManager) since we recompute
    # canonically below.
    _base_for_grossup = Decimal(str(buyer_total_full)) - Decimal(str(bi.stripe_recovery or 0))
    _pp_est = _pp_estimate(
        payment_method="stripe_card" if include_stripe_fee else "offline",
        amount_cents=_to_cents(_base_for_grossup),
        currency=cur,
        payer_role=_PP_ROLE.BUYER,
        jurisdiction=province,
        card_class="domestic",  # updated post-card-confirmation by webhook
        mode="gross_up",
    )
    _pp_snapshot = _pp_est.to_dict()
    _pp_snapshot["amount_cents"] = int(_pp_est.recovery_cents)
    _pp_snapshot["recovery_cents"] = int(_pp_est.recovery_cents)
    _pp_snapshot["estimated_cents"] = int(_pp_est.estimated_cents)
    _pp_snapshot["field_version"] = "payment_processing.v2"
    _canonical_pp_cents = int(_pp_est.recovery_cents)
    _leaked_pp_cents = _to_cents(Decimal(str(bi.stripe_recovery or 0)))
    # Replace whatever was in bi.stripe_recovery with the canonical
    # recovery amount so displayed_total == charge_amount cent-for-cent.
    _canonical_recovery = Decimal(_canonical_pp_cents) / Decimal(100)
    delta = _canonical_recovery - Decimal(str(bi.stripe_recovery or 0))
    ...
    return {
        ...
        "hst": hst,
        "total_tax": total_tax,
        # Stripe — canonical iter482 P5 gross-up recovery.
