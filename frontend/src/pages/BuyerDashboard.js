              ...
              <TabsContent value="wishlist">
                {wishlist && wishlist.length > 0 ? (
                    ...
                    {wishlist.map((item) => {
                      ...
                      return (
                        <Card key={item.wishlist_id} className="overflow-hidden hover:shadow-lg transition-shadow">
                          ...
                          <div className="flex flex-col sm:flex-row gap-4 p-4">
                            <div className="w-full sm:w-32 h-32 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                              {auction.lots?.[0]?.images?.[0] ? (
                                <img src={auction.lots[0].images[0]} alt={auction.title} className="w-full h-full object-cover" />
                              ) : (
                                <div className="w-full h-full flex items-center justify-center text-4xl">📦</div>
                              )}
                            </div>
                            <div className="flex-1 space-y-3">
                              <h3 className="font-bold text-lg line-clamp-2">{auction.title}</h3>
                              <div className="flex flex-wrap gap-2 items-center">
                                <Badge variant="outline" className="text-sm bg-red-50 text-red-600">
                                  <Heart className="h-3 w-3 mr-1 fill-current" />
                                  Wishlisted
                                </Badge>
                                <Badge variant="outline" className="text-sm">{auction.total_lots || 0} lots</Badge>
                                <Badge variant="outline" className="text-sm">{auction.currency}</Badge>
                                {isEnded && <Badge variant="destructive" className="text-sm">Auction Ended</Badge>}
                              </div>
                              <div className="flex gap-4">
                                <div className="p-3 bg-accent/10 rounded-lg flex-1">
                                  <p className="text-xs text-muted-foreground uppercase mb-1">Category</p>
                                  <p className="text-sm font-medium">{auction.category || 'General'}</p>
                                </div>
                                <div className="p-3 bg-accent/10 rounded-lg flex-1">
                                  <p className="text-xs text-muted-foreground uppercase mb-1">Location</p>
                                  <p className="text-sm font-medium">{auction.city}, {auction.region}</p>
                                </div>
                              </div>
                            </div>
                          </div>
                          <CardFooter className="p-4 pt-0 gap-2 flex-col sm:flex-row">
                            <Button 
                              ...
                              onClick={async () => {
                                try {
                                  await axios.delete(`${API}/wishlist/${auction.id}`);
                  ...
                  <Card className="p-12 text-center border-2 border-dashed">
                    <Heart className="h-16 w-16 mx-auto mb-4 text-red-400 opacity-50" />
                    <h3 className="text-xl font-semibold mb-2">Your wishlist is empty</h3>
                    <p className="text-muted-foreground mb-4">
                      Start adding auctions to your wishlist to keep track of items you're interested in
                    </p>
                    <Button className="gradient-button text-white border-0" onClick={() => navigate('/lots')}>
                      Browse Lots Auctions
                    </Button>
                  </Card>
                )}
              </TabsContent>
